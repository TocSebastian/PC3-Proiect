...
if menu_id == 'Home':
    ...
    with hp1:
        ...
        <p class="a">Proiect de cercetare 3 👋  </p>
        """
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    r = requests.get(movie_df['movie_link'].values[0],headers=header)
    doc = BeautifulSoup(r.content, "html.parser")

    images_movies = doc.find(class_="ipc-sub-grid ipc-sub-grid--page-span-2 ipc-sub-grid--wraps-at-above-l ipc-shoveler__grid").find_all(class_="sc-bfec09a1-5 kUzsHJ")[0:7]
    # images_movies
    dict_actors = {'name':[],'img_link':[]}
    for x in images_movies:
        dict_actors['name'].append(x.find(class_="sc-bfec09a1-1 fUguci").text)
        dict_actors['img_link'].append(x.select('div img')[0]['src'])
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # df1_new = pd.read_pickle('df1.pkl')
    # df2_new = pd.read_pickle('df2.pkl')
    # df3_new = pd.read_pickle('df3.pkl')
    # df4_new = pd.read_pickle('df4.pkl')

    # frames = [df1_new,df2_new,df3_new,df4_new]
    ...
    #     frames = [df1pkl,df2pkl,df3pkl,df4pkl]
    ...
    df_prediction1 = pd.read_pickle('df1.pkl')
    df_prediction2 = pd.read_pickle('df2.pkl')
    df_prediction3 = pd.read_pickle('df3.pkl')
    df_prediction4 = pd.read_pickle('df4.pkl')
    
    frames = [df_prediction1,df_prediction2,df_prediction3,df_prediction4]

    df_prediction = pd.concat(frames)

    # df_prediction.iloc[:10,:]['new_id']
    ...
    def Similarity(movieId1, movieId2):
        a = df_prediction.iloc[movieId1]
        b = df_prediction.iloc[movieId2]
        ...
        actorsA = a['actors_bin']
        actorsB = b['actors_bin']
        actorsDistance = spatial.distance.cosine(actorsA, actorsB)
        ...
        RA = a['R_bin']
        RA = b['R_bin']
        RDistance = spatial.distance.cosine(RA, RA)
        
        notaA = a['nota_bin']
        notaB = b['nota_bin']
        notaDistance = spatial.distance.cosine(notaA, notaB)
        
        metascoreA = a['metascore_bin']
        metascoreB = b['metascore_bin']
        metascoreDistance = spatial.distance.cosine(metascoreA, metascoreB)
        ...
        return genreDistance + directDistance + actorsDistance + RDistance + notaDistance + metascoreDistance + yearDistance

    def getNeighbors(baseMovie, K):
            distances = []
        
            for index, movie in df_prediction.iterrows():
    ...
    range_dict = {'small':range(100000), 'medium':range(100000,500000), 'big':range(500000,3000000)}

    def get_category(votes):
        category = []
        for x in range_dict:
            if votes in range_dict[x]:
                category.append(x)
        return category

    #### compare_category function that changes the distance accordingly with the number of votes

    def compare_category(idx,idy,distance):
        
        x = get_category(df_prediction.iloc[idx]['votes_numerical'])
        y = get_category(df_prediction.iloc[idy]['votes_numerical'])
        ...
        elif x[0] == 'big' and y[0] == 'medium':
            distance = distance*0.85
        elif x[0] == 'medium' and y[0] == 'big':
            distance = distance*0.85
        return round(distance,2)
            

    #### convert list to dict function
    ...
    new_movie = df_prediction.loc[movie_df.index[0]].to_frame().T
    # new_movie

    neighbours10 = getNeighbors(new_movie, 10)

    #### convert the list of ids and distances to dictionary

    neighbours_dict10 = convert_list_to_dict(neighbours10)

    #### change the distances accordiing with the category of number of votes that they belong to

    for key in neighbours_dict10:
        neighbours_dict10[key] = compare_category(new_movie.index[0],key, neighbours_dict10[key])

    neighbours_sorted = sorted(neighbours_dict10.items(), key=lambda x:x[1])

    sorted_scores = neighbours_sorted
